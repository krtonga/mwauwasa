...
import com.github.codetanzania.open311.android.library.api.models.ApiAccountRequest;
 ...
 *
 * As of June 6, 2018, in the app module, all network calls (except reporting an issue) use this class.
 *
 * <p>
...
public class MajiFixAPI {
    ...
    private MajifixAccountRetrofitApi initRetrofitAccount() {
        // Configures retrofit
        mRetrofit = new Retrofit.Builder()
                .baseUrl("https://majifix-account.herokuapp.com/")
                .addConverterFactory(GsonConverterFactory.create())
                .addCallAdapterFactory(RxJava2CallAdapterFactory.create())
                .build();

        // Provides majifix account endpoint
        return mRetrofit.create(MajifixAccountRetrofitApi.class);
    }
    ...
    public Single<CustomerAccount> getCustomerAccount(Reporter reporter) {
        System.out.println("Get Customer Account server call started.");
        return initRetrofitAccount()
                .postAccessor(
//                        getAuthToken(),
                        new ApiAccountRequest(reporter))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .map(new Function<ApiCustomerAccount, CustomerAccount>() {
                    @Override
                    public CustomerAccount apply(ApiCustomerAccount apiAccount) {
                        // convert the server object into something the app can use
                        System.out.println("Conversion taking place! " + apiAccount);
                        return ApiModelConverter.convert(apiAccount);
                    }
                });
    ...
    private interface MajifixAccountRetrofitApi {
        @POST("/v1/accounts/verify")
        @Headers({"Accept: application/json", "Content-Type: application/json"})
        Single<ApiCustomerAccount> postAccessor(
//                @Header("Authorization") String authorization,
                @Body ApiAccountRequest accountRequest);
